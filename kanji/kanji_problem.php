...
if (isset($_POST["words"])) {
    ...
    if (count($err_msg)===0){
        if($link!==FALSE){
            $sql='SELECT * FROM kanji order by number'.$limit;
...
<section >
       ...
       <table>
        <tr>
        <td><form action="kanji_index.php">
        <input type="submit" value="問題作成ページ">
        </form></td>
        <td><form action="kanji_answer.php" method="post">
       ...
       <table class="border">
           　　<caption><?php print '漢字　範囲　'.$start.'~'.$end.'まで　　出題数　'.$length ?><?php print '     点数　 /'.$length?></caption>
               <tr class="border">
                   <th class="border">番号</th>
                   <th class="border">訳</th>
                   <th class="border">&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp漢字&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</th>
               </tr>
