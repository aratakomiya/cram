...
if (isset($_POST["words"])) {
    ...
    if (count($err_msg)===0){
        if($link!==FALSE){
            $sql='SELECT * FROM pass_s2 order by number'.$limit;
...
<section >
       ...
       <table>
        <tr>
        <td><form action="pass_s2_index.php">
        <input type="submit" value="問題作成ページ">
        </form></td>
        <td><form action="pass_s2_answer.php" method="post">
       ...
       <table class="border">
           　　<caption><?php print 'パス単2級　範囲　'.$start.'~'.$end.'まで　　出題数　'.$length ?><?php print '     点数　 /'.$length?></caption>
               <tr class="border">
