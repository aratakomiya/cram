...
if (isset($_POST["words"])) {
    ...
    if (count($err_msg)===0){
        if($link!==FALSE){
            $sql='SELECT * FROM junior_ja order by number'.$limit;
...
<section >
       ...
       <table>
        <tr>
        <td><form action="junior_ja_index.php">
        <input type="submit" value="問題作成ページ">
        </form></td>
        <td><form action="junior_ja_answer.php" method="post">
       ...
       <table class="border">
           　　<caption><?php print '中学英単語 日→英　範囲　'.$start.'~'.$end.'まで　　出題数　'.$length ?><?php print '     点数　 /'.$length?></caption>
               <tr class="border">
                   <th class="border">番号</th>
                   <th class="border">日本語</th>
                   <th class="border">&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp英訳&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</th>
               </tr>
