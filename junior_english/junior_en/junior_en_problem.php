...
if (isset($_POST["words"])) {
    ...
    if (count($err_msg)===0){
        if($link!==FALSE){
            $sql='SELECT * FROM junior_en order by number'.$limit;
...
<section >
       ...
       <table>
        ...
        <td><form action="junior_en_answer.php" method="post">
       ...
       <table class="border">
           　　<caption><?php print '中学英単語　英→日　範囲　'.$start.'~'.$end.'まで　　出題数　'.$length ?><?php print '     点数　 /'.$length?></caption>
               <tr class="border">
